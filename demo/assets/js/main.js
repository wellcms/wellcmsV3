...
class WellCMSUI {
  ...
  url(url, extra = {}, config = { path: "./", url_rewrite_on: 0 }) {
    ...
    if (0 == config.url_rewrite_on) {
      r = path + "?" + query + ".html";
    } else if (1 == config.url_rewrite_on) {
      r = path + query + ".html";
    } else if (2 == config.url_rewrite_on || 3 == config.url_rewrite_on) {
      if (query.indexOf("-") === -1) {
        r = (config.path || path) + query;
      } else {
        r =
          (config.path || path) +
          query.replace(/-/g, "/") +
          (2 == config.url_rewrite_on ? ".html" : "");
      }
    }
    ...
    if (extra && Object.keys(extra).length !== 0) {
      ...
      r += sep + new URLSearchParams(extra).toString();
    }
  ...
  async get(url, retry = 1) {
    ...
    try {
      ...
      clearTimeout(timeout);
      if (retry > 1) return this.get(url, retry - 1);
...
class GlobalClickHandler {
  ...
  init() {
    document.addEventListener("click", async (e) => {
      ...
      const argStr = el.getAttribute("data-arg");
      let postData = null;
      if (argStr) {
        try {
      ...
      const executeAction = async () => {
        try {
          ...
          const title =
            res.data?.title || (res.code === 0 ? "Success" : "Error");

          if (res.message) {
            if (res.code === 0 && !res.data?.alert) {
              this.ui.toast(res.message, "success");
            } else {
